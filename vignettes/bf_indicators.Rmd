...
There are 7 indicators related to breastfeeding practices in the updated WHO infant and young child feeding (IYCF) indicator guidelines.

1. Ever breastfeed
2. Early initiation of breastfeeding 
3. Exclusive breastfeeding for the first two days after birth 
4. Exclusive breastfeeding for under six months
5. Mixed milk feeding for under six months 
6. Continuous breastfeeding for 12-23 months 
7. Bottle feeding for 0-23 Months

This article demonstrates how the `{riycf}` package calculates these indicators. The `iycfData` included in the `{riycf}` package will be used for this demonstration.
...
Children less than 24 months who ever breastfed is defined as follows:

$$ \text{Ever breastfed} = \frac{\text{Children}_{\text{age < 24 months}} \text{who ever breastfed}}{\text{Children}_{\text{age < 24 months}}} $$

This indicator can be calculated using the `get_evbf()` function as follows: 
...
Two input parameters are needed to calculate this indicator: *(1) child's age in months*; and, *(2) child was ever breastfed or not*. 
...
evbf <- with(
  df, 
  ifelse(
    calc_age_months < 24 & child_bf == 1, 1,
    ifelse(calc_age_months < 24 & child_bf == 0, 0, NA)
  )
)
...
### Solid or semi-solid food consumption 
...
## Continuous Breastfeeding 12-23 Months (CBF) 
