...
fg_one <- dummy_gen(list(food_1))
fg_two <- dummy_gen(list(food_2))
fg_combined <- dummy_gen(list(food_1, food_2))
...
test_that("Individual `fg_score` output has correct variable type", {
  expect_s3_class(fg_one, "double")
  expect_s3_class(fg_two, "double")
  expect_s3_class(fg_combined, "double")
})
